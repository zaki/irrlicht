	...
	for ( s32 i = 0; i <= dx; i++ )
	{
...
#endif
		{
			...
			dst[i] = PixelAdd32 (
					...
					getTexel_plain ( &IT[1],	d + f32_to_fixPoint ( line.t[1][0].x), 
												d + f32_to_fixPoint ( line.t[1][0].y) )
							);
			...
			getSample_texture ( r1, g1, b1, &IT[1], f32_to_fixPoint ( line.t[0][1].x,inversew), f32_to_fixPoint ( line.t[0][1].y,inversew) );
